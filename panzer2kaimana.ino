...
void setJoystickDirection(int joyDirection)
{
//  int JOY_START = 0;
  int JOY_TOTAL = 16;
//  
//  for (int i = 0; i < 4; i++)
//  {
//    int led = (JOY_START + joyDirection + i) % JOY_TOTAL;
...
//  }
  ...
  for (int i = 0; i < 16; ++i)
  {
    if ((i >= (joyDirection - RANGE) && i < (joyDirection + RANGE))
      || (i >= (joyDirection + JOY_TOTAL - RANGE) || i < (joyDirection - JOY_TOTAL + RANGE)))
    {
      kaimana.setLED(i, LED_JOY_COLOR_ON);
    }
    else
    {
      kaimana.setLED(i, LED_JOY_COLOR_OFF);
    }
  }
}
...
void joystickOff()
{
  for (int i = LED_JOY_START; i <= LED_JOY_END; ++i)
  {
    kaimana.setLED(i, LED_JOY_COLOR_OFF);
  }
}
